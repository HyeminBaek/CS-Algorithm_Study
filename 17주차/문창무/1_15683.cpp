...
void check(vector<vector<int>>& map, vector<vector<int>>& cpymap, int r, int c, int dir) {
    int i = r;
    int j = c;
...
int dfs(vector<vector<int>>& map, vector<CCTV>& cctv, int cctvIdx, vector<int> dir) {
    if (cctv.size() == cctvIdx) {  // 모든 cctv 배치하면 맵 검사
        ...
        for (int i = 0; i < cctv.size(); ++i) {
            ...
            if (type == 1) {  // 1방향
                check(map, cpymap, r, c, dir[i]);
            } else if (type == 2) {  // 2방향 좌우
                check(map, cpymap, r, c, dir[i]);
                check(map, cpymap, r, c, (dir[i] + 2) % 4);
            } else if (type == 3) {  // 2방향 위오른쪽
                check(map, cpymap, r, c, dir[i]);
                check(map, cpymap, r, c, (dir[i] + 1) % 4);
            } else if (type == 4) {  // 3방향
                check(map, cpymap, r, c, dir[i]);
                check(map, cpymap, r, c, (dir[i] + 1) % 4);
                check(map, cpymap, r, c, (dir[i] + 3) % 4);
            } else if (type == 5) {  // 4방향
                check(map, cpymap, r, c, dir[i]);
                check(map, cpymap, r, c, (dir[i] + 1) % 4);
                check(map, cpymap, r, c, (dir[i] + 2) % 4);
